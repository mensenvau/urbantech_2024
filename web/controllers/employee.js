...
exports.employeeHome = fnCatch(async (req, res) => {
    ...
    const [timesheet, requests] = await Promise.all([execute("select * from timesheets where employee_id = ? and date_worked = current_date()", [employee_id], 1), execute("select * from requests where employee_id = ? and active = true order by id desc limit 2", [employee_id])]);
...
exports.employeeLogTimesheet = fnCatch(async (req, res) => {
    ...
    return res.render("employee/main", { data: req.data, page: "timesheets", arr, count, curr });
});
